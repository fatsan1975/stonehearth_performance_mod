...
function Settings:initialize(saved_variables)
   ...
   self._sv.profile = _read_global_config(
      'mods.stonehearth_performance_mod.profile', self._sv.profile)
   self._sv.instrumentation_enabled = _read_global_config(
      'mods.stonehearth_performance_mod.instrumentation_enabled',
      self._sv.instrumentation_enabled) and true or false
end
